...
def visualize_2d_keypoint_registration(global_map_keypoints, scan_keypoints_original, scan_keypoints_transformed, global_map_x_edges, global_map_y_edges, title="2D Keypoint Registration Result"):
    ...
    plt.gca().set_aspect('equal', adjustable='box'); plt.grid(True, linestyle='--', alpha=0.5); plt.show()
...
def visualize_3d_registration_o3d(global_map_points_3d_np, live_scan_points_3d_np, final_transform_4x4, global_map_color=[0.7,0.7,0.7], live_scan_color=[1,0,0]):
    if global_map_points_3d_np.shape[0] == 0 or live_scan_points_3d_np.shape[0] == 0: print("3D 시각화를 위한 포인트가 부족합니다."); return
    pcd_global = o3d.geometry.PointCloud(); pcd_global.points = o3d.utility.Vector3dVector(global_map_points_3d_np); pcd_global.paint_uniform_color(global_map_color)
    pcd_scan = o3d.geometry.PointCloud(); pcd_scan.points = o3d.utility.Vector3dVector(live_scan_points_3d_np); pcd_scan.transform(final_transform_4x4); pcd_scan.paint_uniform_color(live_scan_color)
    print("3D 시각화 실행 중... (창을 닫으면 계속 진행됩니다)")
    o3d.visualization.draw_geometries([pcd_global, pcd_scan], window_name="3D Global Registration Result", width=1024, height=768)
