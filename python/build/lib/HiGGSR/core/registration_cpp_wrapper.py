...
if not os.path.isdir(cpp_build_dir):
    print(f"CRITICAL: Calculated cpp_build_dir does not exist or is not a directory: {cpp_build_dir}")
    # 기존 경로 계산 방식 (폴백 또는 디버깅용)
    fallback_cpp_build_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', '..', 'cpp', 'build'))
    print(f"DEBUG: WRAPPER - Fallback cpp_build_dir calculation was: {fallback_cpp_build_dir}")
...
# C++ 바인딩 모듈 임포트 시도
try:
    print("DEBUG: WRAPPER - Attempting to import HiGGSR_cpp")
    import HiGGSR_cpp
    ...
    print(f"CRITICAL: Could not import C++ extensions for HiGGSR registration ({e}). Wrapper will not function.")
